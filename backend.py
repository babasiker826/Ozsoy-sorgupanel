...
- API Timeout & Headers Optimizasyonu
"""
...
# ----------------------------------------------------------------------------
# API CONFIG - TIMEOUT VE HEADERS
# ----------------------------------------------------------------------------
API_CONFIG = {
    'timeout': 30,  # 30 saniye timeout
    'max_retries': 2,  # Maksimum 2 deneme
    'retry_delay': 1,  # 1 saniye bekleme
    
    # Headers - Gerçek tarayıcı gibi görünmesi için
    'headers': {
...
# ----------------------------------------------------------------------------
# API İSTEK FONKSİYONU - GELİŞTİRİLMİŞ
# ----------------------------------------------------------------------------
def make_api_request(url, method='GET', params=None, data=None, timeout=None, max_retries=None):
    """
    Gelişmiş API istek fonksiyonu
    - Retry mekanizması
    - Timeout kontrolü
    ...
    headers.update({
        ...
        'X-Forwarded-For': g.client_ip if hasattr(g, 'client_ip') else '127.0.0.1',
        'Referer': 'https://panel.ozsoy.app/',
    })
    
    # Session'dan bilgileri al
    if 'key' in session:
        headers['X-API-Key'] = session.get('key', '')[:10] + '...'
    ...
    for attempt in range(max_retries + 1):
        try:
            response = requests.request(
                ...
                stream=False  # Büyük veriler için stream=False
            )
            ...
            if attempt < max_retries:
                time.sleep(API_CONFIG['retry_delay'] * (attempt + 1))  # Artan bekleme süresi
                continue
            else:
                raise Exception(f"API timeout ({timeout}s). Son deneme.")
            ...
            if attempt < max_retries:
                ...
                raise Exception("Bağlantı hatası. API erişilemiyor.")
            ...
            if e.response.status_code == 429:  # Rate limit
                ...
                if attempt < max_retries:
                    ...
                    continue
            raise Exception(f"HTTP hatası: {e.response.status_code}")
    ...
    raise Exception(f"Maksimum deneme sayısına ulaşıldı: {max_retries}")
...
# ----------------------------------------------------------------------------
# GÜNCELLENMİŞ API LISTESI - YENİ API ADRESİ İLE
# ----------------------------------------------------------------------------
APIS = {
    # FREE API'ler (Basit sorgular)
    'tc': {'name': 'TC Sorgulama', 'plan': 'free', 'endpoint': 'https://one01sorguapi.onrender.com/tc?tc={tc}', 'params': ['tc']},
    'yas': {'name': 'Yaş Sorgulama', 'plan': 'free', 'endpoint': 'https://one01sorguapi.onrender.com/yas?tc={tc}', 'params': ['tc']},
    'adsoyad': {'name': 'Ad Soyad Sorgu', 'plan': 'free', 'endpoint': 'https://one01sorguapi.onrender.com/sorgu?ad={ad}&soyad={soyad}', 'params': ['ad', 'soyad']},
    
    # VIP API'ler (Yeni API adresiyle güncellendi)
    # Temel Bilgiler
    'tcyeni': {'name': 'TC Yeni Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/tcyeni?tc={tc}', 'params': ['tc']},
    'adyeni': {'name': 'Ad Soyad Yeni Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/adyeni?ad={ad}&soyad={soyad}', 'params': ['ad', 'soyad']},
    'gsmyeni': {'name': 'GSM Yeni Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/gsmyeni?gsm={gsm}', 'params': ['gsm']},
    
    # Aile ve Akraba API'leri
    'aile': {'name': 'Aile Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/aile?tc={tc}', 'params': ['tc']},
    'sulale': {'name': 'Sülale Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/sulale?tc={tc}', 'params': ['tc']},
    'anne': {'name': 'Anne Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/anne?tc={tc}', 'params': ['tc']},
    'baba': {'name': 'Baba Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/baba?tc={tc}', 'params': ['tc']},
    'kardes': {'name': 'Kardeş Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/kardes?tc={tc}', 'params': ['tc']},
    'cocuk': {'name': 'Çocuk Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/cocuk?tc={tc}', 'params': ['tc']},
    'cocuklar': {'name': 'Çocuklar Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/cocuklar?tc={tc}', 'params': ['tc']},
    'dede': {'name': 'Dede Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/dede?tc={tc}', 'params': ['tc']},
    'nine': {'name': 'Nine Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/nine?tc={tc}', 'params': ['tc']},
    'amca': {'name': 'Amca Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/amca?tc={tc}', 'params': ['tc']},
    'dayi': {'name': 'Dayı Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/dayi?tc={tc}', 'params': ['tc']},
    'hala': {'name': 'Hala Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/hala?tc={tc}', 'params': ['tc']},
    'teyze': {'name': 'Teyze Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/teyze?tc={tc}', 'params': ['tc']},
    'kuzen': {'name': 'Kuzen Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/kuzen?tc={tc}', 'params': ['tc']},
    'yegen': {'name': 'Yeğen Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/yegen?tc={tc}', 'params': ['tc']},
    
    # İletişim ve Adres API'leri
    'tcgsm': {'name': 'TC → GSM Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/tcgsm?tc={tc}', 'params': ['tc']},
    'gsmtc': {'name': 'GSM → TC Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/gsmtc?gsm={gsm}', 'params': ['gsm']},
    'adres': {'name': 'Adres Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/adres?tc={tc}', 'params': ['tc']},
    'gunceladres': {'name': 'Güncel Adres Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/guncelAdres?ad={ad}&soyad={soyad}&tc={tc}', 'params': ['ad', 'soyad', 'tc']},
    
    # Detaylı Bilgi API'leri
    'saglik': {'name': 'Sağlık Bilgileri', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/saglik', 'params': []},
    'cinsiyet': {'name': 'Cinsiyet Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/cinsiyet?tc={tc}', 'params': ['tc']},
    'din': {'name': 'Din Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/din?tc={tc}', 'params': ['tc']},
    'burc': {'name': 'Burç Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/burc?tc={tc}', 'params': ['tc']},
    'medenihal': {'name': 'Medeni Hal Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/medenihal?tc={tc}', 'params': ['tc']},
    'dogumyeri': {'name': 'Doğum Yeri Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/dogumyeri?tc={tc}', 'params': ['tc']},
    'koy': {'name': 'Köy Bilgisi Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/koy?tc={tc}', 'params': ['tc']},
    'vergino': {'name': 'Vergi No Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/vergino?tc={tc}', 'params': ['tc']},
    'kimlikkayit': {'name': 'Kimlik Kayıt Bilgisi', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/kimlikkayit?tc={tc}', 'params': ['tc']},
    
    # İş ve Çalışma API'leri
    'isyerisektoru': {'name': 'İşyeri Sektörü', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/isyeriSektoru?ad={ad}&soyad={soyad}&tc={tc}', 'params': ['ad', 'soyad', 'tc']},
    'isegiristarihi': {'name': 'İşe Giriş Tarihi', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/iseGirisTarihi?ad={ad}&soyad={soyad}&tc={tc}', 'params': ['ad', 'soyad', 'tc']},
    'isyeriunvani': {'name': 'İşyeri Ünvanı', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/isyeriUnvani?ad={ad}&soyad={soyad}&tc={tc}', 'params': ['ad', 'soyad', 'tc']},
    
    # Diğer API'ler
    'tcplaka': {'name': 'TC Plaka Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/tcplaka?tc={tc}', 'params': ['tc']},
    'yetimlik': {'name': 'Yetimlik Bilgisi', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/yetimlik?babatc={babatc}', 'params': ['babatc']},
    'yeniden': {'name': 'Yeniden Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/yeniden?tc={tc}', 'params': ['tc']},
    'olumtarihi': {'name': 'Ölüm Tarihi Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/olumtarihi?tc={tc}', 'params': ['tc']},
    'sms': {'name': 'SMS Bilgisi', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/sms?gsm={gsm}', 'params': ['gsm']},
    'kizliksoyad': {'name': 'Kızlık Soyadı', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/kizliksoyad?tc={tc}', 'params': ['tc']},
    'hikaye': {'name': 'Hikaye Bilgisi', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/hikaye?tc={tc}', 'params': ['tc']},
    'sirano': {'name': 'Sıra No Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/sirano?tc={tc}', 'params': ['tc']},
    'ayakno': {'name': 'Ayak No Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/ayakno?tc={tc}', 'params': ['tc']},
    'operator': {'name': 'Operatör Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/operator?gsm={gsm}', 'params': ['gsm']},
    'yabanci': {'name': 'Yabancı Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/yabanci?ad={ad}&soyad={soyad}', 'params': ['ad', 'soyad']},
    'raw': {'name': 'Raw Veri Sorgu', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/raw?tc={tc}', 'params': ['tc']},
    
    # IBAN API'leri
    'iban_dogrulama': {'name': 'IBAN Doğrulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/iban_dogrulama?iban={iban}', 'params': ['iban']},
    'iban_sorgulama': {'name': 'IBAN Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/iban_sorgulama?iban={iban}', 'params': ['iban']},
    
    # Vesika API'leri
    'vesika': {'name': 'Vesika Sorgulama', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/vesika?tc={tc}', 'params': ['tc']},
    'vesika_download': {'name': 'Vesika İndirme', 'plan': 'vip', 'endpoint': 'https://one01sorguapi.onrender.com/vesika_download?tc={tc}', 'params': ['tc']},
    
    # KORUNAN API'ler (Phishing ve İhbar) - Bunlar eski halinde korunuyor (DOKUNULMADI)
...
def api_sorgu():
    ...
    try:
        # Gelişmiş API istek fonksiyonunu kullan
        result_data = make_api_request(filled_endpoint)
        
        log_audit("API çağrısı başarılı", f"API: {api_id}", key.id)
        
        return jsonify({'success': True, 'data': result_data})
        
    except Exception as e:
        error_msg = str(e)
        log_audit("API hatası", f"API: {api_id}, Hata: {error_msg}", key.id)
        
        # Özel hata mesajları
        if "timeout" in error_msg.lower():
            return jsonify({
                'success': False,
                'error': f'API yanıt vermedi (timeout). Lütfen daha sonra tekrar deneyin.',
                'details': error_msg
            }), 504
        elif "bağlantı hatası" in error_msg.lower():
            return jsonify({
                'success': False,
                'error': f'API sunucusuna bağlanılamıyor. Lütfen daha sonra tekrar deneyin.',
                'details': error_msg
            }), 503
        else:
            return jsonify({
...
@security_middleware
def api_health():
    """API sağlık kontrol endpoint'i"""
    return jsonify({
        'status': 'healthy',
        'timestamp': datetime.now().isoformat(),
        'api_count': len(APIS),
        'active_users': len(user_tracker.user_activity),
...
if __name__ == '__main__':
    ...
    print(f"🔧 API Optimizasyonu: AKTİF")
    print(f"   → Timeout: {API_CONFIG['timeout']}s")
    print(f"   → Retry: {API_CONFIG['max_retries']} deneme")
    print(f"   → Headers: Gerçek tarayıcı emülasyonu")
    ...
    print(f"🔒 Korunan API'ler (Dokunulmadan):")
    print(f"   - İhbar API'leri: 3 adet")
    print(f"   - Phishing API'leri: 8 adet")
    print("="*60 + "\n")
